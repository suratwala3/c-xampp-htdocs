...
class DeliveryController extends Controller
{
    ...
    {
        $deliveryUser = User::where('id', $request->delivery_guy_id)->first();
        ...
        if ($order) {
            ...
            if (!$checkOrder) {
                ...
                $nonCompleteOrders = AcceptDelivery::where('user_id', $request->delivery_guy_id)->where('is_complete', 0)->with('order')->get();
                // dd($nonCompleteOrders->count());
                $countNonCompleteOrders = 0;
                foreach ($nonCompleteOrders as $nonCompleteOrder) {
                    if ($nonCompleteOrder->order->orderstatus_id != 6) {
